...
{
    using namespace gb4e;
    auto applier = Sla<RegisterName::A>;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(Register(RegisterName::A), 0);
    state.SetFlags(0);
...
TEST Instr_Sla_Carry()
{
    using namespace gb4e;
    auto applier = Sla<RegisterName::A>;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(Register(RegisterName::A), 0b10000000);
    state.SetFlags(0);
    ...
    ASSERT(result.GetFlagSet().has_value());
    auto flagSet = result.GetFlagSet().value();
    ASSERT_EQ(FLAG_ZERO | FLAG_C, flagSet.GetValue());
    ASSERT_EQ(1, result.GetRegisterWrites().size());
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(0, regWrite.GetByteValue());
...
{
    using namespace gb4e;
    auto applier = AddD8<RegisterName::A>;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(Register(RegisterName::A), 0);
    state.Set16BitRegisterValue(Register(RegisterName::PC), 0x100);
    memory.Write(0x100, 0xC6);
    memory.Write(0x101, 0);
    state.SetFlags(0);
...
TEST Instr_AddD8_1()
{
    ...
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(1, regWrite.GetByteValue());
    PASS();
}

TEST Instr_CondCall_NoJump()
{
    using namespace gb4e;
    auto applier = CallConditionalA16<FLAG_ZERO, true>;
    GbCpuState state;
    MemoryStateFake memory;
    state.SetFlags(0);
    auto sp = GetRegister(RegisterName::SP);
...
TEST Instr_SubD8_0()
{
    ...
    ASSERT(result.GetFlagSet().has_value());
    auto flagSet = result.GetFlagSet().value();
    ASSERT_EQ(FLAG_ZERO | FLAG_N, flagSet.GetValue());
    ASSERT_EQ(1, result.GetRegisterWrites().size());
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(0, regWrite.GetByteValue());
...
TEST Instr_SubD8_1()
{
    ...
    ASSERT(result.GetFlagSet().has_value());
    auto flagSet = result.GetFlagSet().value();
    ASSERT_EQ(FLAG_N | FLAG_C, flagSet.GetValue());
    ASSERT_EQ(1, result.GetRegisterWrites().size());
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(0xFF, regWrite.GetByteValue());
...
TEST Instr_XorD8()
{
    ...
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(Register(RegisterName::A), 0b11);
    state.Set16BitRegisterValue(Register(RegisterName::PC), 0x100);
    memory.Write(0x100, 0xEE);
    memory.Write(0x101, 0b01);
    state.SetFlags(0);
    ...
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(0b10, regWrite.GetByteValue());
    PASS();
}
...
{
    using namespace gb4e;
    auto applier = SetHL<7>;
    GbCpuState state;
    MemoryStateFake memory;

    state.Set16BitRegisterValue(Register(RegisterName::HL), 0x100);
    memory.Write(0x100, 0b00000001);
    ...
    auto memWrite = result.GetMemoryWrites()[0];
    ASSERT_EQ(0x100, memWrite.GetLocation());
    ASSERT_EQ(0b00000001, memWrite.GetPreviousValue());
    ASSERT_EQ(0b10000001, memWrite.GetValue());
    PASS();
}

TEST Instr_Sbc_NoCarry()
{
    using namespace gb4e;
    auto applier = Sbc<RegisterName::B>;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(GetRegister(RegisterName::A), 100);
    state.Set8BitRegisterValue(GetRegister(RegisterName::B), 10);
...
TEST Instr_Sbc_Carry()
{
    ...
    ASSERT_EQ(RegisterName::A, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(100, regWrite.GetBytePreviousValue());
    ASSERT_EQ(89, regWrite.GetByteValue());
    ASSERT_EQ(1, result.GetConsumedBytes());
    ASSERT_EQ(1, result.GetConsumedCycles());
    PASS();
}

TEST Instr_SbcD8_NoCarry()
{
    using namespace gb4e;
    auto applier = SbcD8;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set8BitRegisterValue(Register(RegisterName::A), 100);
    state.Set16BitRegisterValue(Register(RegisterName::PC), 0x100);
    memory.Write(0x100, 0xDE);
...
TEST Instr_Add16Bit_Carry()
{
    using namespace gb4e;
    auto applier = Add<RegisterName::HL, RegisterName::DE>;
    GbCpuState state;
    MemoryStateFake memory;
    state.Set16BitRegisterValue(GetRegister(RegisterName::HL), 0x2b64);
    state.Set16BitRegisterValue(GetRegister(RegisterName::DE), 0x0038);
    state.SetFlags(0);

    auto result = applier(&state, &memory);

    ASSERT(result.GetFlagSet().has_value());
    printf("flags %x\n", result.GetFlagSet().value().GetValue());
    ASSERT_EQ(0, result.GetFlagSet().value().GetValue());
    ASSERT(result.GetMemoryWrites().empty());
    ASSERT_EQ(1, result.GetRegisterWrites().size());
    auto regWrite = result.GetRegisterWrites()[0];
    ASSERT_EQ(RegisterName::HL, regWrite.GetRegister().GetRegisterName());
    ASSERT_EQ(0x2b64, regWrite.GetWordPreviousValue());
    ASSERT_EQ(0x2b9c, regWrite.GetWordValue());
    ASSERT_EQ(1, result.GetConsumedBytes());
    ASSERT_EQ(2, result.GetConsumedCycles());
    PASS();
}
...
SUITE(Instruction_test)
{
    ...
    RUN_TEST(Instr_Sla_0);
    RUN_TEST(Instr_Sla_1);
    RUN_TEST(Instr_Sla_Carry);
    RUN_TEST(Instr_AddD8_0);
    RUN_TEST(Instr_AddD8_1);
    RUN_TEST(Instr_CondCall_Jump);
    RUN_TEST(Instr_SubD8_0);
    RUN_TEST(Instr_SubD8_1);
    RUN_TEST(Instr_XorD8);
    RUN_TEST(Instr_SetHL);
    RUN_TEST(Instr_Sbc_NoCarry);
    RUN_TEST(Instr_Sbc_Carry);
    RUN_TEST(Instr_SbcD8_NoCarry);
    RUN_TEST(Instr_SbcD8_Carry);
    RUN_TEST(Instr_Add16Bit_Carry);
}
